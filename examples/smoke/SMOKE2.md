...
2. Create the branch `chore/queue-smoke2-<HHMMSS>` (use `date +%H%M%S`) from main.
3. Create the file `smoke/<HHMMSS>.txt` (same suffix) with a single line: `smoke ok <ISO-8601 UTC>`.
4. `git add smoke/` and commit with the message `chore(smoke): queue runner smoke2 <HHMMSS>`.
5. `git push -u origin <branch>`.
6. Open the PR against `main` with `gh pr create --title "chore(smoke): queue runner smoke2 <HHMMSS>" --body "greenlight smoke PR. Manual merge; no real content."`.
